...
import { calculateTarget } from '../utils/calculations';

/* =====================================================================
 * HELPER LOGIC
 * ===================================================================== */
const getActivityMultiplier = (level: string) => level === 'Light' ? 1.1 : level === 'Intense' ? 1.5 : 1.3;
...
const BiologicalBaseline = memo(({ gender, setGender }: { gender: string, setGender: (g: 'Male' | 'Female') => void }) => (
  <View className="mb-6">
    ...
    <View className="flex-row bg-[#F0F9FF] rounded-full p-1 border border-[#E0F2FE]">
      <TouchableOpacity accessibilityRole="radio" accessibilityState={{ checked: gender === 'Male' }} accessibilityLabel="Select Male Gender" onPress={() => setGender('Male')} className={`flex-1 flex-row items-center justify-center py-3 rounded-full ${gender === 'Male' ? 'bg-[#0369A1]' : 'bg-transparent'}`}>
        <Ionicons name="male" size={16} color={gender === 'Male' ? 'white' : '#64748B'} style={{ marginRight: 6 }} />
        <Text className={`font-bold text-sm ${gender === 'Male' ? 'text-white' : '#64748B'}`}>Male</Text>
      </TouchableOpacity>
      <TouchableOpacity accessibilityRole="radio" accessibilityState={{ checked: gender === 'Female' }} accessibilityLabel="Select Female Gender" onPress={() => setGender('Female')} className={`flex-1 flex-row items-center justify-center py-3 rounded-full ${gender === 'Female' ? 'bg-[#0369A1]' : 'bg-transparent'}`}>
        <Ionicons name="female" size={16} color={gender === 'Female' ? 'white' : '#64748B'} style={{ marginRight: 6 }} />
        <Text className={`font-bold text-sm ${gender === 'Female' ? 'text-white' : '#64748B'}`}>Female</Text>
      </TouchableOpacity>
    </View>
  </View>
));
...
const EnergyExpenditure = memo(({ activityLevel, setActivityLevel }: { activityLevel: string, setActivityLevel: (v: string) => void }) => {
  ...
  return (
    <View className="mb-6">
      ...
      {options.map((act, i) => (
        <TouchableOpacity key={i} accessibilityRole="radio" accessibilityState={{ checked: activityLevel === act.label }} accessibilityLabel={`Select ${act.label} activity level`} onPress={() => setActivityLevel(act.label)} className={`flex-row items-center p-4 rounded-[20px] mb-3 ${activityLevel === act.label ? 'bg-[#0EA5E9]' : 'bg-[#F4F8FB] border border-[#E2E8F0]'}`}>
          ...
          <View className="flex-1">
            <Text className={`font-black text-sm mb-0.5 ${activityLevel === act.label ? 'text-white' : '#0F172A'}`}>{act.label}</Text>
            <Text className={`font-semibold text-[11px] ${activityLevel === act.label ? 'text-[#E0F2FE]' : 'text-slate-500'}`}>{act.desc}</Text>
          </View>
          <Text className={`font-black text-sm ${activityLevel === act.label ? 'text-white' : '#0F172A'}`}>{act.mult}x</Text>
        </TouchableOpacity>
      ))}
    </View>
  );
...
export default function Profile() {
  ...
  const currentTarget = calculateTarget(weight, parseInt(age) || 25, getActivityMultiplier(activityLevel), gender);

  const handleSave = async () => {
    Keyboard.dismiss();
    const finalWake = wakeUp.trim().length === 5 ? wakeUp : '07:00';
    const finalBed = bedtime.trim().length === 5 ? bedtime : '23:00';
